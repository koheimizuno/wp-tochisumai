...
document.addEventListener("DOMContentLoaded", function () {
  ...
  firstTabLink && firstTabLink.classList.add("active");
  // 最初のコンテンツを表示
...
jQuery(document).ready(function ($) {
  var $wantArea = jQuery(".want-area");
  var $areaSelect = jQuery(".area-select");
  ...
  $wantArea.each(function () {
    ...
    getAreaByWantArea(jQuery(this));
    jQuery("#area-select01").val(postTitle);
...
function getAreaByWantArea(item) {
  let selectedArea = item.val();
  let areaSelect = item.closest(".d-flex").find(".area-select");
  areaSelect.html('<option value="">選択してください</option>');
  switch (selectedArea) {
    case "茨城県北茨城市":
      areaSelect.append(
...
jQuery(function ($) {
  // 初期状態で全てのフォームを非表示
  $(".area-form01").show();
  $(".other-form, .hope-area-wrapper").hide();
  $(".want-schedule").hide();
  ...
  $('input[name="contact"]').change(function () {
    ...
    if (selectedValue === "資料請求したい") {
      ...
      $(".area-form01").hide();
      $(".want-schedule").show();
    } else if (selectedValue === "見学したい") {
      $(".hope-area-wrapper").show();
      $(".area-form01").hide();
      $(".want-schedule").show();
    }
...
function logTitle(title, area) {
  const wantArea = jQuery("#want-area01");
  if (area) wantArea.val(area).trigger("change");
  if (title) {
    jQuery("#hope-area").val(title);
    postTitle = title;
  }
}
