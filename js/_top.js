...
jQuery(window).on('load', function() {
	jQuery(function($) {
		setTimeout(function() {
		$('#loading_box').delay(1300).queue(function() {
			$(this).addClass('loading_logo__scale');
		})
		$('#loading').delay(1500).queue(function() {
			$(this).addClass('loading_fadeOut');
		})
		$('#loading-bar').delay(1500).queue(function() {
			$(this).addClass('loading__finish');
		})
		$("#loading_stoker").delay(1500).fadeOut('slow');
		$('.main-slider').delay(2500).queue(function() {
			$(this).addClass('main_visual__fadeIn');
		})
		$('.fv_text_main').delay(3300).queue(function() {
			$(this).addClass('header_pc__fadeIn');
		})
        $('.fv_text_sub').delay(3000).queue(function() {
			$(this).addClass('header_pc__fadeIn');
		})
        $('.header, .nav_btn_wrap').delay(3500).queue(function() {
			$(this).addClass('header_pc__fadeIn');
		})
...
jQuery(window).on('load', function() {
    jQuery(function($) {
    setTimeout(function() {
      var $slider = $('.home .main-slider').slick({
        arrows: false,
        slidesToShow: 1,
        slidesToScroll: 1,
        fade: true,
...
function handleScroll(e) {
    if (isScrolling) {
        ...
        if (e.deltaY > 0 && currentPanelIndex < panels.length - 1) {
            // マウスホイールが下に動いたとき
            // 最初のパネルが完全に表示されるまで次のパネルに移動しない
            if (currentPanelIndex === 0 && window.scrollY < offsetTop) {
                // 最初のパネルのテキストをフェードイン
                return;
            }
            // 次のパネルに移動
...
aboutWrap.addEventListener('touchstart', function(e) {
...
aboutWrap.addEventListener('touchend', function(e) {
    if (touchEndY < touchStartY && currentPanelIndex < panels.length - 1) {
        // 下にスクロール
        currentPanelIndex++;
        goToPanel(panels[currentPanelIndex]);
    } else if (touchEndY > touchStartY && currentPanelIndex > 0) {
        // 上にスクロール
        currentPanelIndex--;
        goToPanel(panels[currentPanelIndex]);
    }
