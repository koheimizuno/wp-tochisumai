...
<?php if ( ! is_front_page() ) : ?>
  ...
  if($myupload_images) {
		// 配列を5つに制限
		$myupload_images = array_slice($myupload_images, 0, 5);
  ...
  <?php echo $myupload_div_l; ?>
	...
	<div class="mv_loader opa">
		<div class="mv_loader_line">
			<svg class="pc_disp tb_disp"><circle cx="70" cy="70" r="69"></svg>
			<svg class="sp_disp"><circle cx="45" cy="45" r="44"></circle></svg>
		</div>
		<div class="mv_loader_text">
			<p class="kerning en0">
				<span><span>0</span></span>
				<span>
				<!-- <span class="mv_loader_num mv_loader_num0 opa">1</span>
			  	<span class="mv_loader_num mv_loader_num1 opa">2</span>
			  	<span class="mv_loader_num mv_loader_num2 opa">3</span>
			  	<span class="mv_loader_num mv_loader_num3 opa">4</span> -->
				<?php
                if ($myupload_images) {
                    foreach ($myupload_images as $key => $img_id) {
                        echo '<span class="mv_loader_num mv_loader_num' . $key . ' opa">' . ($key + 1) . '</span>';
                    }
                }
                ?>
			  </span>
...
?>
				...
				<h1 class="case_text_sub"><?php the_title(); ?></h1>
			</div>
    </div>
  </div>

  <script>
    var slideNum = <?php echo $myupload_images ? count($myupload_images) - 1 : 0; ?>;
  </script>
